...
<<<<<<< HEAD
4.  Make sure that the output directory exists (the default is `/tmp/alphafold`)
...
5.  Run `run_docker.py` pointing to a FASTA file containing the protein
...
<<<<<<< HEAD
6.  You can control which AlphaFold model to run by adding the
...
7.  You can control MSA speed/quality tradeoff by adding
